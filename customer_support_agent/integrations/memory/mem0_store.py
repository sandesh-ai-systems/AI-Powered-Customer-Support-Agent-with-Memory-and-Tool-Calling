### This is system’s long-term memory manager (stores and retrieves past experiences)
...
class CustomerMemoryStore:
    ...
    def search(self, query: str, user_id: str, limit: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            raw = self._memory.search(query, filters=filters, top_k=limit)
        except TypeError:
    ...
    def list_memories(self, user_id: str, limit: int = 20) -> list[dict[str, Any]]:
        if hasattr(self._memory, "get_all"):
            filters = {"user_id": user_id}
            try:
                raw = self._memory.get_all(filters=filters, top_k=limit)
            except TypeError:
