...
class SupportCopilot:
    ...
    def generate_draft(self, ticket: dict[str, Any], customer: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            runtime_errors.append(f"Knowledge base search failed: {exc}")
        ...
        try:
            ...
            runtime_errors.append(f"Primary agent generation failed: {exc}")
        used_fallback = False
        if not draft_text:
        ...
        if runtime_errors:
            context_used.setdefault("errors", []).extend(runtime_errors)
        if used_fallback:
    ...
    def _search_memory_scopes(
        ...
        for scope_user_id in scope_user_ids:
            try:
                ...
                self._memory_error = f"Search failed for {scope_user_id}: {exc}"
                continue
            raw_hits.extend(self._annotate_memory_scope(hits=hits, scope_user_id=scope_user_id))
